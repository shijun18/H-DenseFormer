...
def get_paths_to_patient_files(path_to_imgs, append_mask=True):
    ...
    """
    path_to_imgs = pathlib.Path(path_to_imgs)  #resample之后的图片
...
def get_train_val_paths(all_paths, path_to_train_val_pkl):
    ...
    train_paths = [path for path in all_paths
                   if any(patient_id + '_ct.nii.gz' in str(path[0]) for patient_id in train_val_split['train'])] #按照CT进行了一次划分
...
def get_attributes(sitk_image):  
    # """Get physical space attributes (meta-data) of the image."""  meta-data 通常以“数据头”的格式被储存在文件的开头，包括患者ID信息、名字以及图像矩阵维度、空间分辨率、像素深度和光度表示等信息光度
    attributes = {}
